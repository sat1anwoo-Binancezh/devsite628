...
- 点击“开始监听”后，先建立当前邮件基线，只提醒之后新收到的邮件。
- 开始监听后先建立当前邮件基线；只推送监听开始之后新收到的邮件。
- 每小时 00、15、30、45 分进入 1 分钟扫描窗口，窗口内每 2 秒持续扫描。
- 窗口内发现新邮件后会继续等待后续邮件；连续两轮没有新增后马上合并成一个托盘通知和一个强制确认弹窗。
